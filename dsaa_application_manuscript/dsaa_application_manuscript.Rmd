...
```{r osm-features}
tbl_all <- data_dublin_2018_features %>%
  dplyr::select(price, any_of(osm_dublin_features))

tbl_y <- tbl_all %>%
  dplyr::select(price)

train_tbl_y <- tbl_y[train_index, ]
test_tbl_y <- tbl_y[test_index, ]

tbl_x <- tbl_all %>%
  dplyr::select(-price)

train_tbl_x <- tbl_x[train_index, ]
test_tbl_x <- tbl_x[test_index, ]

xgmodel <- xgboost::xgboost(
  data = train_tbl_x %>% as.matrix(),
  label = train_tbl_y %>% as.matrix(),
...
pred <- predict(xgmodel, test_tbl_x %>% as.matrix())
...
# library(iBreakDown)
# 
# xgmodel <- parsnip::boost_tree(mode = "regression") %>%
#   parsnip::set_engine(engine = "xgboost") %>%
#   parsnip::fit_xy(x = train_tbl_x, y = train_tbl_y)
# 
...
# library(lime)
# explainer <- lime::lime(tbl_all, model = xgmodel)
# explanation <- lime::explain(tbl_x[1:5,], explainer, n_features = 2)
