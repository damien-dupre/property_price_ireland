...
```{r distrib-plot,fig.width=12, fig.cap="Distribution of the PSRA database (filtered)."}
data_dublin_geocoded_clean %>% 
...
gam_pred %>%
  ...
  scale_fill_gradientn(
    ...
    trans = "log",
    breaks = c(250000, 400000, 600000, 1000000), 
...
source(here::here("script/AIRO_dublin_features.R"))
#
...
xgb.importance(model = xgmodel) %>% 
  ...
  kable(
    "latex",
    caption = "Census features importance (higher than 1%).",
    booktabs = T,
    linesep = ""
  ) %>%
  kable_styling(font_size = 8,
...
```{r all_OSM_features, results="asis"}
