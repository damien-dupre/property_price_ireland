...
```{r distrib-plot,fig.width=4, fig.height=4, fig.cap="Distribution of the PSRA database (filtered).", out.extra=''}
data_dublin_geocoded_clean %>% 
...
The average properties price is `r round(mean_dat,0)` euros (SD = `r round(sd_dat,0)`). In order to remove potential human errors and outliers, prices higher or lower than 1 SD were removed from the original dataset.

```{r density-plot,fig.width=4, fig.height=4, fig.cap="Geographical density of the PSRA database (filtered).", out.extra=''}
autoplot(dublin_map) +
...
In order to model the distibution of properties, a Generalized Additive Model using gaussian scale family for Soap film smooths was calculated to fit the price of properties sold according to their GPS coordinates. The result indicates that `r scales::percent(res_gam_dublin_gps$dev.expl)` of property prices is explained by property localisation (*F*(`r res_gam_dublin_gps$n`,`r round(res_gam_dublin_gps$s.table[,"edf"],2)`) = `r round(res_gam_dublin_gps$s.table[,"F"], 2)`; *p* `r format.pval(res_gam_dublin_gps$s.table[,"p-value"], eps = 0.001)`).

```{r gam-plot,fig.width=4, fig.height=4, fig.cap="Prediction of property price according the GAM model. Prediction too far from the actual data were removed to avoid unrealistic extrapolation.", out.extra=''}
...
The prediction accuracy using the XGBoost algorithm reaches a very high level (`r lm_result$full_result$modelfit$r2`).

```{r OSM-features-xgb, fig.height=5, fig.cap="Property price prediction accuracy (left) and Property price prediction error (right) using geographical features with XGBoost.", out.extra=''}
...
```{r census-features-xgb, fig.height=5, fig.cap="Property price prediction accuracy (left) and Property price prediction error (right) using socio-economic features with XGBoost.", out.extra=''}
result_accuracy <- result %>%
  ggplot(aes(x = price, y = pred)) +
  stat_density2d(geom = "tile",
                 aes(fill = ..density.. ^ 0.20),
...
read_csv(here::here("data/census_dublin_features.csv")) %>% 
  kable(
    "latex",
